...
public interface PersonaStatsRepository extends JpaRepository<PersonaStatsEntity, Long> {
    ...
    @Query(value = "SELECT RANK FROM " +
            ...
            "WHERE PS.VERS = ?2 AND PS.PLAYTIME > 0 AND P.DELETED_ON IS NULL) AS STATS " +
            "WHERE STATS.PERSONA_ID = ?1", nativeQuery = true)
    Long getRankByPersonaIdAndVers(long id, String vers);

    @Query(value = "FROM PersonaStatsEntity ps WHERE ps.vers = :vers AND ps.playTime > 0 AND ps.persona.deletedOn IS NULL ORDER BY (kill - death) DESC, persona.id ASC LIMIT :limit OFFSET :offset")
    List<PersonaStatsEntity> getLeaderboardByVers(String vers, long limit, long offset);

    @Query(value = "FROM PersonaStatsEntity ps WHERE ps.vers = :vers AND ps.playTime > 0 AND ps.persona.deletedOn IS NULL ORDER BY kill DESC, persona.id ASC LIMIT :limit OFFSET :offset")
